...
describe('reduced motion styles', () => {
  ...
  it('turns the wisp parade into a static three-character scene', () => {
    expect(homeCss).toMatch(
      /@media \(prefers-reduced-motion: reduce\)[\s\S]*\.hero-wisp\s*\{[^}]*animation: none !important/s,
    )
    expect(homeCss).toMatch(/\.hero-wisp:nth-child\(-n \+ 3\)\s*\{[^}]*display: block/s)
    expect(homeCss).toMatch(
      /\.hero-wisp__particle,[\s\S]*\.hero-wisp__body::before\s*\{[^}]*animation: none !important/s,
    )
    expect(homeCss).toMatch(
      /\.hero-wisp__particle:nth-child\(1\),[\s\S]*\.hero-wisp__particle:nth-child\(2\)\s*\{[^}]*display: block/s,
    )
