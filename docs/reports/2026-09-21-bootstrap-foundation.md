...
実装を止める未決事項はない。今後の運用について次を確認したい。

1. `AGENTS.md`の「Bootstrapは限定的に使う」という旧方針を、今回のBootstrap基盤へ改めてよいか。今回は明示依頼を優先して実装し、恒久指示ファイル自体は未編集。
2. 新規ブランチのpushでCloudflareの自動プレビューデプロイが実行されない設定か。originは`leftonbo/leftonbo.com`だが、リポジトリ内に自動デプロイのブランチ設定がなく、pushは保留。公開・deploy・mergeは未実施。
