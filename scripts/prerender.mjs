...
function validateHead(head, route) {
  ...
  for (const key of ['url', 'alt', 'mimeType']) {
    if (typeof head.socialImage[key] !== 'string' || head.socialImage[key].trim().length === 0) {
...
function renderHeadTags(head, jsonLd, route) {
  ...
  if (safeHead.publishedTime !== undefined) {
    tags.splice(
      -1,
      0,
      `<meta property="article:published_time" content="${escapeHtml(safeHead.publishedTime)}">`,
    )
  }
