...
async function verifyMachineReadableFiles() {
  ...
  try {
    ...
    fail(`data/works.json is not valid JSON: ${error instanceof Error ? error.message : String(error)}`);
  }
...
async function main() {
  ...
  console.log(`[verify-dist] verified ${htmlCount} HTML files, data/works.json, llms.txt, and 404.html`);
}
